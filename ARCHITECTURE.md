...
│   network drop) is NEVER terminal. It auto-pauses the recording      │
│   and the session remains recoverable. Either participant can         │
│   rejoin and resume.                                                 │
│                                                                      │
...
│      → FE calls POST /leave (instant auto-pause if recording)        │
│                                                                      │
...
│      → Backend auto-pauses if recording                              │
│                                                                      │
...
│                                                         │
│  ── Recording State ──                                  │
│  recording_id, recording_segments                       │
...
**We use**: `room_name` → session_id, `s3_key` → store for processing pipeline, `recording_id` → verify match. This is NOT used for pause/resume/leave logic — purely for S3 data capture + safety net reconciliation.
...
| recording | paused | FE POST /pause (host only) | conditional: status = recording |
| recording | paused | FE POST /leave OR webhook participant.left | participant leaves during recording → auto-pause |
| paused | recording | FE POST /resume (host only) | conditional: status = paused AND set size >= 2 |
...
| recording/paused | processing | FE POST /end (host only, explicit) | conditional: status IN (recording, paused). **ONLY terminal user action.** |
...
│                                                                  │
│  2. LEAVE during recording = auto-pause.                         │
│     - Backend stops Daily.co recording (to save the segment)     │
│     - Status moves to "paused" (NOT processing)                  │
...
│     - Recording is paused, room is alive (2hr TTL)               │
...
| Mute/unmute | **SDK** (local only) | Per-device, no need to persist |
...
# → Read set size → auto-pause if recording and count < 2
   ...
   BLOCKING                           Validate: status = ready
                                      ...
                                      Conditional update:          ──→  status: recording
                                        SET status=recording,           recording_id: "rec-xyz"
                                            recording_id,               recording_segments: 1
                                            recording_segments=1,       recording_started_at: now
                                            recording_started_at=now,   version: 1
                                            version=version+1
...
GUEST SIDE:
   - Daily SDK fires "recording-stopped" → triggers immediate poll
   - GET /sessions/{id} → status: paused
   - UI: "Recording paused by host"

WEBHOOK (recording.ready-to-download, fires later when files hit S3):
   → Re-read session → status=paused → pause-induced stop → SKIP
   → Store s3_key if present (useful for processing pipeline later)
```
...
2. POST /sessions/{id}/resume    ──→  Read session
   BLOCKING                           Validate: status = paused
                                      ...
                                      Call Daily.co:
                                        start_recording(room_name)
                                      Conditional update:          ──→  status: recording
                                        SET status=recording,           recording_segments: 2
                                            recording_segments += 1,    version: 3
                                            version=version+1
                                        CONDITION: status = paused
                                 ←──  { status: "recording" }

3. Dispatch STATUS_UPDATED → resume timer, recording UI
...
```
1. First to leave during recording → auto-pause, set drops to 1
2. Second to leave → set drops to 0, status stays paused
3. Daily room stays alive (2hr TTL)
4. Either can rejoin within 2 hours
5. If nobody returns: Daily room expires, DynamoDB TTL cleans up after 30 days
```
...
LAMBDA:
  1. List tracks for session
  2. If < 2 tracks → return (wait for next S3 event)
  3. If >= 2: download, convert, merge, upload processed files
...
│     Then: if count < 2 during recording → AUTO-PAUSE             │
...
│     Reconciliation: if status still = recording (our DynamoDB    │
│       write after stop_recording() failed), move to paused       │
│     Guard: SKIP if status == paused (pause-induced stop, s3_key  │
│       still stored)                                              │
...
| **Explicit "Leave" button** | No | FE `POST /leave` (immediate) | **Instant** | Yes → auto-pause |
| **Tab close (Cmd+W)** | No | Daily webhook after heartbeat timeout | **10-30s** | Yes → auto-pause |
| **Browser close (Cmd+Q)** | No | Daily webhook after heartbeat timeout | **10-30s** | Yes → auto-pause |
| **Browser crash** | No | Daily webhook after heartbeat timeout | **10-30s** | Yes → auto-pause |
| **Device shutdown** | No | Daily webhook after heartbeat timeout | **10-30s** | Yes → auto-pause |
| **Network blip (<10s)** | No | **Nothing** — Daily SDK auto-reconnects | **No pause** | **No** |
| **Network down (>30s)** | No | Daily webhook after heartbeat timeout | **10-30s** | Yes → auto-pause |
...
| File | Changes |
|------|---------|
| `api/app/models/session.py` | Remove `participant_count`. Add `active_participants: set[str]`, `participant_connections: dict[str,str]`, `participants: dict[str,str]` (roster), `version: int`, `last_pause_at: Optional[str]`, `s3_key: Optional[str]`. Update serialization. |
| `api/app/repos/session_repo.py` | Add `add_participant(session_id, user_id, connection_id, user_name)` — atomic ADD+SET. Add `remove_participant(session_id, user_id)` — atomic DELETE+REMOVE. Add `conditional_update_status()`. Remove `increment_participant_count()`. |
| `api/app/services/session_service.py` | Join/leave accept `user_id`, `connection_id`, `user_name`. Leave triggers auto-pause if recording. Webhook `on_participant_left` checks connection map for staleness (compares stored connection vs webhook's `session_id`). Replace `on_recording_stopped` with `on_recording_ready_to_download` (stores `s3_key`, reconciles if stuck in recording). Remove `on_recording_started` (no room field in payload). Start requires status=ready. Resume requires set size >= 2. Rename stop→end. |
...
| `api/app/types/responses.py` | Add `participant_count` (derived from set size), `active_participants: list[str]`, `participants: dict[str,str]` (roster), `s3_key: Optional[str]`. |
...
| File | Changes |
|------|---------|
| `web/src/hooks/useDaily.ts` | Expose local participant's `session_id` (Daily's connection ID) and `user_id` (from token) after join. SDK events trigger `onSdkEvent` callback for poll triggering. Remove `isRecording` from state (server owns this). |
...
| `web/src/pages/AudioRoom.tsx` | Server-driven rendering. SDK events trigger polls. No sendBeacon/beforeunload. Disconnect banner when participant leaves during recording. Token from sessionStorage. Initial GET before join. Redirect to complete if terminal status. |
...
| `web/src/types/session.ts` | Add `active_participants: string[]`, `participants: Record<string, string>`, `participant_count: number` (derived), `s3_key: string | null`. |
